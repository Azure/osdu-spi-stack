...
- **No stored credentials.** Workload Identity is the only path for Azure PaaS
  access: local auth is disabled on Cosmos and Service Bus, and their Key Vault
  key/connection secrets are `DISABLED` placeholders (ADR-027). Never commit
  secrets or hardcode credentials.
