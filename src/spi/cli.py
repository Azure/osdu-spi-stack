...
def onboard(
    ...
    if not list_trusted and (repo is not None and not repo.strip()):
        raise typer.BadParameter("--repo must be <owner>/<name>", param_hint="--repo")
    ...
    try:
        ...
        if remove:
            ...
            if not write:
                _onboard.render_remove_plan(plan)
                return
            rows = _onboard.apply_remove(plan)
            _onboard.render_rows(rows, f"Trust removed for {service}")
            return
        plan = _onboard.plan_onboard(
            target, service, repo or "", org=org or "", skip_repo=skip_repo
        )
        if not write:
