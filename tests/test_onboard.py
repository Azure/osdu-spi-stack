...
class TestPlanning:
    def test_a_fresh_repository_plans_all_three_phases_in_order(self):
        ...
        assert [s.phase for s in plan.steps] == ["repository"] * 8 + ["azure", "cluster"]
        assert verbs(plan)[:3] == [
            "gh api --method PUT",
            "gh api --method POST",
            "gh api --method POST",
        ...
        assert verbs(plan)[-2] == "az identity federated-credential create"
        assert (
            plan.steps[-1].argv[-1]
    ...
    def test_a_correct_repository_only_restamps_the_unreadable_secret(self):
        state = State((cred("partition", REPO),), {"partition": REPO}, PROTECTED, correct_values())
    ...
    def test_a_drifted_variable_gets_one_set_and_reads_drifted(self):
        ...
        plan = make_plan(State((cred("partition", REPO),), {"partition": REPO}, PROTECTED, values))
    ...
    def test_remove_plans_revoke_then_reprojection(self):
        ...
        plan = make_plan(
            State(roster, {"partition": REPO, "schema": "Acme/osdu-spi-schema"}),
            repo="",
            remove=True,
        )
...
class Live:
    ...
    def observed(self, *, values: bool = True) -> State:
        ...
        return State(
            self.roster, self.projection, self.protection, dict(self.values) if values else {}
        )
    ...
    def run(self, argv, **_):
        ...
        if argv[:3] == ["az", "identity", "federated-credential"]:
            ...
            others = tuple(c for c in self.roster if c.service != service)
            if argv[3] == "delete":
                self.roster = others
            else:
                repo = argv[argv.index("--subject") + 1].split(":")[1]
                self.roster = others + (cred(service, repo),)
...
def live(monkeypatch):
    ...
    monkeypatch.setattr(onboard, "read_roster", lambda target: world.roster)
...
class TestApply:
    ...
    def test_a_busy_identity_is_retried_after_backoff(self, live):
        ...
        assert [c[3] for c in live.calls if c[0] == "az"] == ["create", "create"]
        assert live.roster == (cred("partition", REPO),)
        assert {row.state for row in rows} == {"correct"}
    ...
    def test_remove_revokes_then_reprojects(self, live):
        ...
        assert [c[3] for c in live.calls] == ["delete"]
        assert live.projection == {"schema": "Acme/osdu-spi-schema"}
        assert [(r.state, r.detail) for r in rows][0] == ("correct", "absent")

    def test_nothing_to_change_touches_nothing(self, live):
        live.roster = (cred("partition", REPO),)
