...
- The UAMI is declared in `infra/modules/identity.bicep` and receives RBAC role assignments via `infra/modules/rbac.bicep`: Key Vault Secrets User, Storage Blob Data Contributor, Storage Table Data Contributor, Service Bus Data Sender, Service Bus Data Receiver, AcrPull. The kubelet identity also holds AcrPull, and is the identity pods pull through. The Cosmos modules add the Cosmos DB Built-in Data Contributor data-plane role on each partition SQL account (`partition.bicep`) and on the Gremlin account (`cosmos-gremlin.bicep`).
