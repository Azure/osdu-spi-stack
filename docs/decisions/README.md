...
| [008](008-bicep-for-azure-provisioning.md) | Bicep for Azure Provisioning | Azure resources are declared in hand-written Bicep at pinned API versions; the CLI owns the lifecycle seams: resource group, Key Vault soft-delete recovery, kubeconfig, CNI chaining, deployer cluster-admin grant, K8s bootstrap, runtime secrets. |
...
| [018](018-karpenter-nodepool-authoring.md) | Karpenter NodePool Authoring as Workload Manifests | NodePools and AKSNodeClasses are Flux-managed manifests: tainted `platform` and `osdu` pools selected by the stack-owned `spi-pool` label. `platform` is fixed-shape, on-demand, `WhenEmpty`; `osdu` is a size range, spot with on-demand fallback, consolidated when underutilized; stateful members carry required hostname anti-affinity. |
...
| [030](030-machine-readable-status-contract.md) | Machine-Readable Status and the Deploy Record | `spi status --json` emits a versioned envelope whose exit code gates on `deployable`, with `ready` for convergence as a field; the running version is read from what Flux applied plus a release-stamped ConfigMap, and `spi up` records its own run in RG tags and a `spi-deploy-record` ConfigMap. |
...
| [032](032-environment-deploy-identity.md) | Environment Deploy Identity and Namespace RBAC | `spi up` provisions one deploy identity per environment; `spi onboard` trusts a repository by adding a federated credential for its protected deploy environment; two Roles bind the identity through the cluster config, reads split from a `resourceNames`-scoped lock write, no create, delete, or secrets verbs. |
| [033](033-explicit-canonical-image-source-policy.md) | Explicit Canonical Image Source Policy | Credentials record repository trust; retained RG tags record explicit canonical-source policy and project it into the image lock. Reviewed declarations own both on declared environments; onboarding alone does not promote a fork image. |
| [034](034-deploy-identity-survives-down.md) | Managed Identities Survive `spi down` | `spi down` keeps managed identities, the group, its tags, and external grants; `--purge` confirms external-grant cleanup before deleting the group and identities. |
| [035](035-deployer-permission-model.md) | Deployer Permission Model and Preflight Check | The deployer holds Contributor plus Role Based Access Control Administrator conditioned to the stack's eleven roles at subscription scope; `spi check`, `spi up`, and `spi down --purge` read ARM permissions and stop before the first change when one is missing. |
| [036](036-spi-test-runs-the-deployed-commit.md) | `spi test` Runs the Deployed Commit's Own Suites | `spi test` runs a service's suite from the acceptance image paired with the deployed commit, bound by that commit's own descriptor and resolver fetched from the trusted fork, or natively from a checkout with `--source`; the resolver sees only an allowlisted environment. |
