...
| [032](032-per-fork-deploy-identity.md) | Environment Deploy Identity and Namespace RBAC | `spi up` provisions one deploy identity per environment; `spi onboard` trusts a repository by adding a federated credential for its protected deploy environment; two Roles bind the identity through the cluster config, reads split from a `resourceNames`-scoped lock write, no create, delete, or secrets verbs. |
| [033](033-canonical-image-source-follows-onboarding.md) | Canonical Image Source Follows Onboarding | Credentials record repository trust; retained RG tags record explicit canonical-source policy and project it into the image lock. Reviewed declarations own both on declared environments; onboarding alone does not promote a fork image. |
| [034](034-deploy-identity-survives-down.md) | Managed Identities Survive `spi down` | `spi down` deletes resources individually and keeps managed identities, the group, and its tags, so a rebuild never rotates the client id forks hold; `--purge` deletes the group. |
