...
| [030](030-machine-readable-status-contract.md) | Machine-Readable Status and the Deploy Record | `spi status --json` emits a versioned envelope with 0/2/1 exit codes, `ready` for convergence and `deployable` as the deploy gate; `spi up` records the deployed version in RG tags and a `spi-deploy-record` ConfigMap. |
