...
| [033](033-canonical-image-source-follows-onboarding.md) | Canonical Image Source Follows Onboarding | `spi onboard` records a service's source in the environment's image lock, flipping its canonical image from community GitLab to the fork's GHCR `main` line one service at a time. |
