...
| [015](015-partition-entitlements-bootstrap.md) | Partition + Entitlements Bootstrap via a Flux Helm Chart | A Flux Helm chart renders per-partition one-shot Jobs that create the partition record and provision entitlements root groups. |
...
| [029](029-environment-lifecycle-and-reset-boundary.md) | Environment Lifecycle Verbs and the Reset Boundary | Five fixed verbs: upgrade is `spi up` re-run at the new tag, the weekly Saturday full reset is the only reset, and refresh is workflow composition over `spi reconcile`. |
| [030](030-machine-readable-status-contract.md) | Machine-Readable Status and the Deploy Record | `spi status --json` emits a versioned envelope with 0/2/1 exit codes; `spi up` records the deployed version in RG tags and a `spi-deploy-record` ConfigMap carrying the `maintenance` flag. |
| [031](031-fork-image-deploys-as-ephemeral-pins.md) | Fork-Built Images Deploy as Ephemeral Lock Pins | A fork deploy is a GHCR digest pin on the image lock marked ephemeral: verified against the running pod, restored by an always-run reset, swept by the weekday backstop, with canonical flipping per service via `github_repo`. |
| [032](032-per-fork-deploy-identity.md) | Per-Fork Deploy Identity and Namespace RBAC | Each fork gets a UAMI with three federated credentials; cluster access binds through one Entra group to two namespace Roles with no create, delete, or secrets verbs, provisioned by `spi onboard`. |
