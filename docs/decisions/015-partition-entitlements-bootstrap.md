...
- Reference data, legal tags, and adding human users to `users.datalake.ops` remain out of scope; add later if the stack needs user-facing UI access or record ingestion. (Amendment: legal tags landed later through the same chart's `legal-init` Job, which stages the COO catalog into the partition's `legal-service-azure-configuration` container and creates the `{partition}-demo-legaltag` default tag; same auth pattern, same 201/409 idempotency rule.)
