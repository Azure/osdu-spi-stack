...
- `legal-init` prints one typed `legal-init outcome: <code>` line on every exit, so a failure is diagnosable from the Job log without re-running it. It is not a Helm hook: seeding a legal tag is not a readiness gate, and `ready` and `seeded` stay separate signals.
...
- Manual re-run is `kubectl delete job -n osdu partition-init-{p} entitlements-init-{p} legal-init-{p}` followed by `flux reconcile kustomization spi-osdu-init`. 409 responses on re-run are treated as success.
