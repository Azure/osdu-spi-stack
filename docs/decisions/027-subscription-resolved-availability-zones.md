...
template, so an override receives the same preflight validation, the
catalogue's `EphemeralOSDiskSupported` capability included; local disk
capacity remains ARM-validated.
