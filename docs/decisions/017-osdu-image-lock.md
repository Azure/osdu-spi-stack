...
- `src/spi/images.py` resolves each service in `IMAGE_REGISTRY` from its explicit canonical-source policy (ADR-033), not from repository trust. `spi up` loads that policy from the reviewed declaration or retained RG source tags before image resolution; standing-cluster refresh reads its projection in the lock's per-service source keys. With no selected fork the service is GitLab-origin and resolution queries the community registry API for the newest immutable SHA tag on the configured branch (default `master`); a promoted service resolves its fork's GHCR `main` image instead. The schema-load image is resolved from the selected schema-service SHA and fails fast if that exact loader tag is absent.
