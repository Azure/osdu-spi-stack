...
- `src/spi/images.py` resolves each service in `IMAGE_REGISTRY` from the source the environment records for it (ADR-033), read from the lock's per-service source keys. With no recorded fork the service is GitLab-origin and resolution queries the community registry API for the newest immutable SHA tag on the configured branch (default `master`); a service recorded against a fork resolves that fork's GHCR `main` image instead. The schema-load image is resolved from the selected schema-service SHA and fails fast if that exact loader tag is absent.
