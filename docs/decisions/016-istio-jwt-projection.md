...
- Sidecar injection is a prerequisite for this ADR to execute at all. The `osdu` namespace `istio.io/rev` label is sourced from the live cluster revision via `osdu-flux/spi-cluster-config` and Flux substitution, so Git does not pin an outdated ASM revision.
