...
[ADR-027](../decisions/027-entra-only-data-plane.md) disables local (SAS) auth on every Service Bus namespace, so `{partition}-sb-connection` now holds the literal `"DISABLED"` like the other data-plane secrets; no SAS string exists to hand out. Until indexer-queue runs a Workload-Identity-capable image (delivered through the pinned custom-image supply chain, ADR-032), it cannot subscribe to `recordstopic` and records-changed events do not reach the indexer. Restoring that path is tracked with the custom-image work, not by re-enabling local auth.
