...
| upgrade | `env-upgrade` | push to `main` touching the pin file, dispatch | 2 h |
| reset | `env-reset` | Saturday cron 06:00 UTC, confirm-dispatch | 4 h |
| teardown | `env-teardown` | protected dispatch | 1 h |

All four (unbuilt) share concurrency group `env-shared` with
`cancel-in-progress: false`, so lifecycle operations serialize; fork deploys
serialize per service and are otherwise concurrent (ADR-031). The workflows
